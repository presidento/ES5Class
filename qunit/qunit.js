...
Test.prototype = {
	...
	setup: function() {
		if (this.module != config.previousModule) {
			...
			config.moduleStats = { all: 0, bad: 0, warning: 0 };
	...
	finish: function() {
		...
		if ( tests ) {
			...
			for ( var i = 0; i < this.assertions.length; i++ ) {
				...
				if ( !assertion.result && assertion.isWarning ) {
				...
				if ( assertion.result ) {
					...
					if ( assertion.isWarning ) {
						warning++;
						config.stats.warning++;
						config.moduleStats.warning++;
					}
				}
			}
			...
			for ( var i = 0; i < this.assertions.length; i++ ) {
				if ( !this.assertions[i].result ) {
					...
					config.stats.bad++;
					config.moduleStats.bad++;
					if ( this.assertions[i].isWarning ) {
						warning++;
						config.stats.warning++;
						config.moduleStats.warning++;
					}
...
var QUnit = {
	...
	ok: function(a, msg) {
		...
		config.current.assertions.push({
			result: a,
			isWarning: (config.current.nextIsWarning--) > 0,
			message: msg
		});
	},
	
	warn: function(isWarn) {
		if ( typeof isWarn ) {
			isWarn = true
		}
		config.current.nextIsWarning = isWarn ? 1 : 0;
		return this;
	},
...
extend(QUnit, {
	...
	init: function() {
		extend(config, {
			stats: { all: 0, bad: 0, warning: 0 },
			moduleStats: { all: 0, bad: 0, warning: 0 },
			started: +new Date,
			updateRate: 1000,
			blocking: false,
			autostart: true,
...
function done() {
	...
	var banner = id("qunit-banner"),
		...
		'<span class="passed">', config.stats.all - config.stats.bad, '</span> tests of <span class="total">', config.stats.all, '</span> passed, <span class="failed">', config.stats.bad,'</span> failed (', config.stats.warning, ' warning).'].join('');

	if ( banner ) {
		if ( config.stats.bad ) {
			if ( config.stats.bad === config.stats.warning ) {
				banner.className = "qunit-warning";
			} else {
				banner.className = "qunit-fail";
			}
		} else {
